...
{	
	...
	while(true){				// Se crea un bucle que siempre corre cuando el sistema operativo lo asigna
		...
		vTaskDelay(READING_DISTANCE_DELAY/portTICK_PERIOD_MS);		// Delay para realizar la tarea relacionada con la medición de la distancia y el manejo de la tapa
	}
}
...
{
	bool trash_full=false;		// Variable que indica si se alcanzó el peso máximo, requerido para realizar la desinfección y que no se esté desinfectando continuamente si previamente no se alcanzó el peso máximo
	...
	while(true){				// Se crea un bucle que siempre corre cuando el sistema operativo lo asigna
		...
		UartSendString(UART_PC,(const char*)UartItoa(weight,10));
		UartSendString(UART_PC," \r\n");	// Se envía por puerto serie el valor que retorna la balanza, utilizado para calibrar y definir el umbral del peso máximo
		...
		vTaskDelay(READING_WEIGHT_DELAY/portTICK_PERIOD_MS);	// Delay para realizar la tarea relacionada con la medición del peso y el manejo de los leds y el desinfectador
	}
}
